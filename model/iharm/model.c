...
void try_set_model_parameter(const char *word, const char *value)
{
  ...
  //set_by_word_val(word, value, "q", &q, TYPE_DBL);
...
double get_SPO2023_qshear(double r){
  // 1st attempt using central difference formula
  /*double dOmegadr = radial_derivative(i, j, k, Omega); //grid.c, taking Omega as data[n]->p[U2][i][j][k]
  double r = (i + 0.5) * dx[1];
  double qshear = - r * dOmegadr / Omega[i][j][k];

  fprintf(stderr, "dOmegadr: %g, Omega: %g, r: %g\n", dOmegadr, Omega[i][j][k], r);;*/


  //qshear from S. Dogan & C. J. Nixon (2020), ref from Gammie 2004 and Penna et al. 2013
  double qshear = 1.5 * (1. - 2. / r + a * a * pow(r, -2)) / (1. - 3. / r + 2. * a * pow(r, -1.5));

  return qshear;
}

double get_SPO2023_nR(double Ur, double r_isco, double r){
  double eta = 0.1; //arbitrary parameter
  double nR = log(Ur / eta) / log(r_isco / r); // intentionally(?) ignore a negative sign here
  return nR;
}
...
void init_physical_quantities(int n, double rescale_factor)
{
  ...
  // copy from /model/riaf/model.c, calulate r_isco
    ...
    for (int j = 0; j < N2+2; j++) {
      for (int k = 0; k < N3+2; k++) {
        ...
        if (ELECTRONS == ELECTRON_DUMP) {
          ...
          double r = i * dx[1] + startx[1];
          double qshear = get_SPO2023_qshear(r);
          double nR = get_SPO2023_nR(data[n]->p[U1][i][j][k], r_isco, r);
          ...
          //debug
          if (isnan(trat)) fprintf(stderr, "Ur = %.5f, r: %.5f, r_isco: %.5f, qshear: %.5f, nR: %.5f, trat: %.5f\n", data[n]->p[U1][i][j][k], r, r_isco, qshear, nR, trat);
